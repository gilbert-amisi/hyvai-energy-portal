...
import { sendQuoteRequest } from '@/services/productService';
...
export default function QuoteModal({ product, onClose }: { product: any, onClose: () => void }) {
  const [formData, setFormData] = useState({
    ...
    Quantity: '',
    Message: '',
    Products: product ? [product.id] : [], // Relation ManyWay avec le produit actuel
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      Products: product?.documentId ? [product.documentId] : [],
    ...
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
      <div className="bg-white w-full max-w-lg rounded-[2rem] p-10 shadow-2xl relative overflow-hidden">
        ...
        <div className="mb-8">
          ...
          <p className="text-zinc-400 text-xs text-center font-bold uppercase tracking-widest mt-1">
            Requesting for: <span className="text-blue-600">{product?.Item_Number}</span>
          </p>
        </div>
